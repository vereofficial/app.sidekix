  ...
  return (
    <View style={[styles.flex, { backgroundColor: colors.bg, paddingTop: insets.top }]}>
      <ScrollView contentContainerStyle={{ padding: 18, gap: 14, paddingBottom: Math.max(insets.bottom, 24) }}>
        <Text style={[styles.title, { color: colors.text1, fontFamily: font.serifItalic }]}>before you start</Text>
        <Text style={[styles.sub, { color: colors.text2, fontFamily: font.dm }]}>
          submit one sidequest idea for approval, then pick your default home style.
        </Text>
        <TextInput
          value={idea}
        ...
        <View style={styles.row}>
          {CATEGORIES.map((c) => (
            ...
            >
              <Text style={{ color: colors.text2, fontFamily: font.mono, fontSize: 11 }}>{c}</Text>
            </Pressable>
          ))}
        </View>
        <Text style={{ color: colors.text3, fontFamily: font.mono, fontSize: 11, textTransform: 'uppercase', letterSpacing: 0.8 }}>
          preferred view
        </Text>
        <View style={[styles.modeWrap, { backgroundColor: colors.pillBg }]}>
          <Pressable onPress={() => setMode('feed')} style={[styles.modeBtn, mode === 'feed' && { backgroundColor: colors.accent }]}>
            <Text style={{ color: mode === 'feed' ? (resolvedScheme === 'light' ? '#fff' : '#0A0A0A') : colors.text3, fontFamily: font.dmBold }}>sidequest view</Text>
          </Pressable>
          <Pressable onPress={() => setMode('recent')} style={[styles.modeBtn, mode === 'recent' && { backgroundColor: colors.accent }]}>
            <Text style={{ color: mode === 'recent' ? (resolvedScheme === 'light' ? '#fff' : '#0A0A0A') : colors.text3, fontFamily: font.dmBold }}>feed view</Text>
          </Pressable>
        </View>
        <Text style={[styles.note, { color: colors.text2, fontFamily: font.dm }]}>
          feed view = stream of posts/ideas/reactions · sidequest view = ideas grouped with adventures nested under them.
        </Text>
        <Text style={[styles.note, { color: colors.text3, fontFamily: font.dm }]}>
          your first onboarding idea posts immediately.
        </Text>
        <Pressable disabled={!canFinish} onPress={() => void finish()} style={[styles.finishBtn, { backgroundColor: canFinish ? colors.accent : colors.bg3 }]}>
          <Text style={{ color: canFinish ? (resolvedScheme === 'light' ? '#fff' : '#0A0A0A') : colors.text3, fontFamily: font.dmBold }}>
