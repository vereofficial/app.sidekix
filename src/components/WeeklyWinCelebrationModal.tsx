...
import { countRankedBelowYou, type WeeklyWinCelebrationPayload } from '../lib/weeklyWinCelebration';
import { openSidekixInstagramDm } from '../lib/openSidekixInstagramDm';
import { font, getColors } from '../theme';

type Props = {
  visible: boolean;
  payload: WeeklyWinCelebrationPayload | null;
  onClose: () => void;
};

export function WeeklyWinCelebrationModal({ visible, payload, onClose }: Props) {
  ...
  const primaryBtnSub = scheme === 'light' ? 'rgba(255,255,255,0.9)' : 'rgba(10,10,10,0.82)';
  ...
  const title =
    payload.variant === 'prize'
      ? 'You won the week'
      : payload.variant === 'first_no_pool'
        ? 'You finished #1'
        : 'You topped the week';

  const subtitle =
    payload.variant === 'prize'
      ? 'Most reactions last week — you locked it in. Claim your prize on Instagram.'
      : payload.variant === 'first_no_pool'
        ? 'The prize pool hadn’t unlocked yet, but you still ran the board. Huge.'
        : 'You had the most reactions, but the prize needs a few more qualifying reactions on others’ posts. Tap below for details.';

  const { recap } = payload;
  /** Win moments are always for finishing 1st. */
  const edgedOutCount = countRankedBelowYou(recap.competitorCount, 1);
      ...
      <View style={[styles.root, { backgroundColor: colors.bg }]}>
        ...
        >
          <Animated.View style={{ opacity: fade, transform: [{ scale }] }}>
            ...
            <Text style={[styles.title, { color: colors.text1, fontFamily: font.syneExtra }]}>{title}</Text>
            <Text style={[styles.subtitle, { color: colors.text2, fontFamily: font.dm }]}>{subtitle}</Text>
            ...
            >
              <Text style={[styles.primaryBtnText, { color: primaryBtnLabel, fontFamily: font.syne }]}>DM @sidekix.app</Text>
              <Text style={[styles.primarySub, { color: primaryBtnSub, fontFamily: font.dm }]}>
                Send proof to claim · Venmo after we verify
              </Text>
            </Pressable>
